...
main(int argc, char **argv)
{
    ...
    auto workspaceGroup = xcscheme::SchemeGroup::Open(&filesystem, ProcessContext::GetDefault()->userName(), workspace->basePath(), workspace->projectFile(), workspace->name());
    ...
    ForEachFileRef(workspace,
            [&](xcworkspace::XC::Group const *g, xcworkspace::XC::FileRef const &fref)
            {
                ...
                auto project = PBX::Project::Open(&filesystem, path);
                if (project) {
                    auto projectGroup = xcscheme::SchemeGroup::Open(&filesystem, ProcessContext::GetDefault()->userName(), project->basePath(), project->projectFile(), project->name());
                    if (projectGroup) {
                        schemes.insert(schemes.end(),
